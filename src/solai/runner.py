...
def doctor():
    ...
    if not shutil.which("swerex-remote"):
        print("✗ SWE-ReX not found - did pip install succeed?")
        sys.exit(1)
    else:
        print("✓ SWE-ReX")
    ...
    for cmd, name in core_checks:
        try:
            ...
            print(f"✗ {name} check failed. Please ensure it's installed and in PATH.")
            sys.exit(1)
    ...
    if cfg.exists():
        ...
        if not image_present(tag):
            print("✗ Image tag missing – run `solai image-rebuild`")
            sys.exit(1)
...
def rebuild_image():
    ...
    subprocess.run(["docker", "build", "-t", img_tag, "-f", str(here), "."], check=True)
...
def run_backlog(cfg_path: Path, once: bool, max_conc: int, log_path: Path):
    ...
    if not re.match(r"[a-z0-9\-]+-\d{4}-\d{2}-\d{2}", model):
        print("Model string must pin date (e.g., gpt4o-2025-04-25)")
        return
    ...
    def _worker():
        with tempfile.TemporaryDirectory(prefix="solai-") as tmp:
            ...
            _run(
                ["git", "-C", str(workdir), "checkout", "-b", cfg["task"]["branch"]],
                Path("."),
                None,
            )
...
""")
                ...
                _run(
                    [
                        os.getenv("SWE_REX_BIN", "swerex-remote"),
                        "run",
                        "--image",
                        cfg["env"]["docker_image"],
                        "--",
                        "sweagent",
                        "run",
                        "--config",
                ...
                diff_files = list(workdir.glob("*.diff")) + list(
                    workdir.glob("*.patch")
                )
                if not diff_files:
                    print("No diff inside patch tar")
                    return
                diff = diff_files[0]
